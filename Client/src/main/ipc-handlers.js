...
function registerIpcHandlers(dbManager, mainWindow) {
  ...
  ipcMain.handle('file:delete', async (event, { fileId }) => {
    ...
    try {
      ...
      // TODO: Implement server-side file deletion endpoint
      // await api.deleteFile(file.storagePath, currentUser.token);
  ...
  ipcMain.handle('files:sync', async () => {
    ...
    try {
      const serverFiles = await api.syncFiles(currentUser.token);
      // TODO: Implement sync logic to merge server and local files
